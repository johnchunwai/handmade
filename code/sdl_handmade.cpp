...
internal void sdl_cleanup(SDL_Window *window, SDL_Renderer *renderer, SDL_Texture *texture)
{
    if (texture)
    {
...
int main(int argc, char **argv)
{
    ...
    // create back buffer
    if (!sdl_resize_backbuffer(&g_backbuffer, renderer, backbuffer_width, backbuffer_height))
    {
        sdl_cleanup(window, renderer, g_backbuffer.texture);
        return 1;
    }
